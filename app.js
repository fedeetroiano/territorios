...
function renderPlanilla() {
    ...
    appState.ultimoGenerado.asignaciones.forEach((row, idx) => {
        ...
        let diaHtml = row.diaNombre ? `<td class="dia-col" rowspan="${rowspan}"><div style="text-align: center; width: 100%; display: block;">${row.diaNombre}</div></td>` : '';
...
function exportarImagen() {
    ...
    // Forzar ancho fijo generoso para que html2canvas no aplaste la tabla si la ventana es pequeña
    ...
    // Convert inputs to plain text elements for a clean screenshot without borders
    ...
    inputs.forEach(i => {
        let text = '';
        if (i.tagName === 'SELECT') {
            text = i.options[i.selectedIndex] ? i.options[i.selectedIndex].text : '';
        } else if (i.tagName === 'DIV') {
            text = i.innerText;
        } else {
            text = i.value;
        }
        ...
        // Hide original input
        const originalDisplay = i.style.display;
        i.style.display = 'none';
        
        replacements.push({ input: i, span: span, display: originalDisplay });
    });

    // Opciones para asegurar que no se recorte por la ventana del navegador
    const opt = { 
        scale: 2.5, 
        logging: false,
    ...
    html2canvas(el, opt).then(canvas => {
        // Restaurar elementos y quitar spans temporales
        replacements.forEach(r => {
            r.input.style.display = r.display;
            r.span.remove();
        });
